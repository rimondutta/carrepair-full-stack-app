...
<body style="margin: 0; padding: 0; background-color: #f4f4f4; font-family: Arial, sans-serif;">
  <div style="padding: 20px 10px; background-color: #f4f4f4;">
    <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 10px; overflow: hidden; border: 1px solid #dddddd;">
      <!-- Header -->
      <div style="background-color: #EAB308; padding: 20px; text-align: center; color: #ffffff;">
        <h1 style="margin: 0; text-transform: uppercase; letter-spacing: 2px; font-size: 24px;">Sam's Automotive Repairs</h1>
        <p style="margin: 5px 0 0; font-size: 14px; opacity: 0.8;">MOT Testing & Car Repair - Jamaica, NY</p>
      </div>
      ...
      <div style="background-color: #f9f9f9; padding: 20px; text-align: center; color: #999999; font-size: 12px; border-top: 1px solid #eeeeee;">
        <p style="margin: 0;">123-30 Merrick Blvd, Jamaica, NY 11434, United States</p>
        <p style="margin: 5px 0 0;">© ${new Date().getFullYear()} Sam's Automotive Repairs. All rights reserved.</p>
      </div>
    </div>
  </div>
</body>
...
export const sendBookingConfirmation = async (booking: IBooking) => {
  ...
  const mailOptions = {
    ...
    to: email,
    subject: `Booking Confirmed: ${serviceType} - Sam's Automotive Repairs`,
    html: getEmailLayout(`
      ...
      <div style="background-color: #f9f9f9; border-left: 4px solid #EAB308; padding: 15px; margin: 25px 0;">
      ...
      <div style="text-align: center; margin-top: 35px;">
        <a href="tel:+1 (718) 481-9800" class="mobile-full" style="background-color: #EAB308; color: white; padding: 14px 28px; text-decoration: none; font-weight: bold; border-radius: 6px; text-transform: uppercase; font-size: 14px; display: inline-block;">Call Us: +1 (718) 481-9800</a>
      </div>
    `),
  };
...
export const sendBookingCancellation = async (booking: IBooking) => {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || '"Sam\'s Automotive Repairs" <noreply@samsautomotiverepairs.com>',
    to: email,
    subject: `Booking Cancelled: ${serviceType} - Sam's Automotive Repairs`,
    html: getEmailLayout(`
      <h2 style="color: #333; margin-top: 0; font-size: 20px;">Hello ${customerName},</h2>
      <p style="color: #666; font-size: 16px;">
...
export const sendBookingCompletion = async (booking: IBooking) => {
  ...
  const mailOptions = {
    from: process.env.SMTP_FROM || '"Sam\'s Automotive Repairs" <noreply@samsautomotiverepairs.com>',
    to: email,
    subject: `Service Completed: ${serviceType} - Sam's Automotive Repairs`,
    html: getEmailLayout(`
      <h2 style="color: #333; margin-top: 0; font-size: 20px;">Hello ${customerName},</h2>
      <p style="color: #666; font-size: 16px;">
      ...
      <p style="color: #666; font-size: 16px;">
        Thank you for choosing <strong>Sam's Automotive Repairs</strong>. We look forward to serving you again!
      </p>
    `),
  };
