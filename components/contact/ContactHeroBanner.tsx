...
export default function ContactHeroBanner({ title, breadcrumbs }: ContactHeroBannerProps) {
  return (
    <section className="relative w-full min-h-[400px] pt-[110px] bg-gradient-to-r from-[#110E10] via-[#1B1B1B] to-[#110E10] flex items-center overflow-hidden border-b-2 border-[#79AD43]">
      ...
      <div className="container mx-auto px-4 md:px-8 relative z-10">
        <div className="flex flex-col items-start">
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="w-8 h-[2px] bg-[#79AD43]"></div>
            <span className="text-[#79AD43] text-xs font-black uppercase tracking-[0.3em] heading-font">
              CONTACT US
            </span>
            <div className="w-8 h-[2px] bg-[#79AD43]"></div>
          </div>
              ...
              return (
                <div key={idx} className="flex items-center gap-4">
                  {crumb.href && !isLast ? (
                    <Link 
                      href={crumb.href}
                      className="text-[#999] text-xs font-bold uppercase tracking-widest hover:text-[#79AD43] transition-colors"
                    >
                      {crumb.label}
                    </Link>
                  ) : (
                  ...
                  {!isLast && (
                    <div className="flex items-center opacity-40">
                      <ChevronRight className="w-3 h-3 text-[#79AD43]" />
                    </div>
                  )}
                </div>
              );
