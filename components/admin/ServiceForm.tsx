...
const inputCls =
  'w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-[#EAB308]/20 focus:border-[#EAB308] outline-none transition-all text-slate-900 placeholder:text-slate-300 text-sm';
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl border border-slate-200 p-6 mb-8">
      ...
      <Section title="Basic Info">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" name="isActive" id="isActive" checked={form.isActive} onChange={handleField} className="w-4 h-4 text-[#EAB308] rounded border-gray-300 focus:ring-[#EAB308]" />
      ...
      <Section title="Page Content Paragraphs">
        ...
        <button type="button" onClick={() => addArrayItem('detailedContent')} className="flex items-center gap-1.5 text-xs font-bold text-[#EAB308] hover:text-[#A80003] transition-colors mt-1">
        ...
        {form.checklist.map((item, i) => (
          <div key={i} className="flex gap-2 items-center">
            ...
            {form.checklist.length > 1 && (
              <button type="button" onClick={() => removeArrayItem('checklist', i)} className="text-rose-400 hover:text-[#EAB308] transition-colors">
                <Trash2 size={16} />
              </button>
            )}
          </div>
        ))}
        <button type="button" onClick={() => addArrayItem('checklist')} className="flex items-center gap-1.5 text-xs font-bold text-[#EAB308] hover:text-[#A80003] transition-colors mt-1">
        ...
        {form.iconBoxes.map((box, i) => (
          <div key={i} className="p-4 bg-slate-50 rounded-xl space-y-3 relative">
            ...
            {form.iconBoxes.length > 1 && (
              <button type="button" onClick={() => removeIconBox(i)} className="absolute top-3 right-3 text-rose-400 hover:text-[#EAB308] transition-colors">
                <Trash2 size={15} />
              </button>
            )}
          </div>
        ))}
        <button type="button" onClick={addIconBox} className="flex items-center gap-1.5 text-xs font-bold text-[#EAB308] hover:text-[#A80003] transition-colors">
      ...
      <Section title="Service FAQs">
        ...
        {form.faqs.map((faq, i) => (
          <div key={i} className="p-4 bg-slate-50 rounded-xl space-y-3 relative">
            ...
            {form.faqs.length > 1 && (
              <button type="button" onClick={() => removeFaq(i)} className="absolute top-3 right-3 text-rose-400 hover:text-[#EAB308] transition-colors">
                <Trash2 size={15} />
              </button>
            )}
          </div>
      ...
      <div className="flex gap-3 mt-6 pt-5 border-t border-gray-200">
        <button
          type="submit"
          disabled={loading}
          className="px-8 py-3 bg-[#EAB308] shadow-lg shadow-[#EAB308]/20 text-white rounded-xl font-bold hover:bg-[#A80003] hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:translate-y-0"
        >
